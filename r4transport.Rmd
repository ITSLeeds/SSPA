...
3. When we looked at this data before in QGIS, we were interested in the 'Supergroup' variable. From exploring the data above, we can see that `SPRGRP` is variable `[6]`.

If we wanted to plot this, we would used the command:

```{r}
plot(harrogate_oac[6])
```

Alternatively, we could call the `SPRGRP` variable directly:

```{r}
plot(harrogate_oac["SPRGRP"])
```
...
That may seem complicated and confusing, but it's actually a highly efficient way of subsetting geographical data. Remember the long-winded QGIS way of doing it, for example:
