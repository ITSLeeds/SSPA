...
date: "`r Sys.Date()` for SSPA TRAN5015."
output: 
...
These can be downloaded in a single `.zip` file from: [github.com/ITSLeeds/SSPA/archive/master.zip](https://github.com/ITSLeeds/SSPA/archive/master.zip). Unzip the folder and open the file `SSPA.Rproj` with RStudio.
...
The data we will used is stored in the `data` folder.
...
```{r, message=FALSE}
school_file = "data/HGTJTSSecondary_School.csv"
...
The results tell us some very useful things about this dataset: all the records are in the same city (Harrogate again) and there is a wide variability in the time it takes to get to school by bicycle. Refer back to the data in `data/jts0503.xls` to see what the column names mean.

```{r}
variable_names = readxl::read_excel("data/jts0503.xls", skip = 9)
variable_names
```
...
Use the command `?cor` to find out what the output means: that the Pearson coefficient of determination is very close to 1, meaning that there is a strong positive relationship between the time taken to get to school by car and bike.
...
```{r}
zones_file = "data/HGT/HGT_JTS_SecondarySchShape.shp"
...
```{r, out.width="50%", fig.cap="Illustration of facet plotting with the `plot` command.", warning=FALSE}
plot(zones[6:10])
```
...
You can do this by joining the data using the `inner_join()` function from the **dplyr** package. Before the data can be joined, we must ensure the 'joining variable' has the same name in both datasets:

```{r, out.width="50%"}
library(dplyr)
names(school_data)[1] # recall name of LSOA variable
names(zones)[1]
school_data = rename(school_data, LSOA11C = LSOA_code)
```
...
```{r, out.width="50%"}
zones_joined = left_join(x = zones, y = school_data)
